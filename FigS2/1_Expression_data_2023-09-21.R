...
#LOAD
setwd("~/Library/CloudStorage/OneDrive-NorwichBioScienceInstitutes/DAP-seq/2022-10-06 NAC TF background")
#setwd("C:/Users/evansc/OneDrive - Norwich BioScience Institutes/DAP-seq/2022-10-06 NAC TF background")

expression <- read_delim("heatmap_21_9_2023_tpm_raw.tsv", delim = "\t", skip = 13)
target_genes <- read_excel("NAC_target_genes_2023-09-21.xlsx")
...
expression_long <- expression %>%
  pivot_longer(cols = 13:ncol(expression), names_to = "Gene stable ID", values_to = "tpm") %>%
...
plot <- ggplot(senescence, aes(x = as.numeric(DPA), y = tpm, col = Subgenome, group = `Gene name`)) +
  #geom_point() +
  facet_wrap(vars(Triad), labeller = label_wrap_gen(width = 20)) +
...
for(triad in unique(senescence$Triad)){
  ...
  ggsave(filename = paste("NAC_thesis_tpm_", triad, "_", today(), ".svg", sep = ""), plot,
